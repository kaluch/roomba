...
public class CapteurDistance extends Capteur {
	...
	public void miseAjour(Environnement environ) {
		...
		for(Obstacle x : environ.getObstacles())
		{
			...
			else if(x.getForme() instanceof Carre || x.getForme() instanceof Rectangle)
				distTemp3 = Math.min(distTemp3,contactRectangleCarre(x.getForme()));
		}
	...
	private double contactRectangleCarre(Forme forme){
		...
		Vecteur2D O = new Vecteur2D(this.getForme().getPosture().getX(),getForme().getPosture().getY());
		Vecteur2D P = new Vecteur2D(this.getForme().getPosture().getX()+10000*Math.cos(this.getForme().getPosture().getTheta()),
									this.getForme().getPosture().getY()+10000*Math.sin(this.getForme().getPosture().getTheta()));
		Vecteur2D A = new Vecteur2D(forme.getPosture().getX()-largeur/2,forme.getPosture().getY()-hauteur/2);
		Vecteur2D B = new Vecteur2D(forme.getPosture().getX()-largeur/2,forme.getPosture().getY()+hauteur/2);
		Vecteur2D C = new Vecteur2D(forme.getPosture().getX()+largeur/2,forme.getPosture().getY()+hauteur/2);
		Vecteur2D D = new Vecteur2D(forme.getPosture().getX()+largeur/2,forme.getPosture().getY()-hauteur/2);
	...
	private double distanceSegSeg(Vecteur2D A,Vecteur2D B,Vecteur2D O, Vecteur2D P){
	...
	private double contactCercle(Cercle cercle){
		double distTemp = 10;
				...
				if(ci>=cercle.getDiametre()/2)
					...
					Vecteur2D bc = new Vecteur2D(cercle.getPosture().getX()-forme.getPosture().getX()+1000*Math.cos(forme.getPosture().getTheta()),
							cercle.getPosture().getY()-forme.getPosture().getY()+1000*Math.sin(forme.getPosture().getTheta()));
					...
					if((pscal1 >= 0 && pscal2 >=0)|| ac.norme() < cercle.getDiametre()/2 ){
						...
						if (ac.norme() < cercle.getDiametre()/2  ){
							...
							if(som.norme()>=dif.norme())
								norme = ai.norme() + Math.sqrt(Math.pow(cercle.getDiametre()/2, 2)-
									Math.pow(cif.norme(),2));
							else
								norme =-( ai.norme() - Math.sqrt(Math.pow(cercle.getDiametre()/2, 2)-
										Math.pow(cif.norme(),2)));
