...
public class Carre extends Forme implements Cloneable {
	...
	void paint(Graphics2D g2, int facteurEchelle, int centrex, int centrey) {
		g2.fillRect(centrex + getxPix(facteurEchelle)
				- (int) (cote * facteurEchelle / 2), centrey
				+ getyPix(facteurEchelle) - (int) (cote * facteurEchelle / 2),
				(int) (facteurEchelle * cote), (int) (facteurEchelle * cote));
	}
	...
	public boolean collisionInterne(Cercle cercle) {
		return ((cercle.getPosture().getY() - cercle.getDiametre() / 2 <= getPosture()
				...
				|| (cercle.getPosture().getY() + cercle.getDiametre() / 2 >= getPosture()
						.getY() + getCote() / 2) || (cercle.getPosture().getX()
				+ cercle.getDiametre() / 2 >= getPosture().getX() + getCote()
				/ 2)) ? true : false;
	...
	public boolean collisionExterne(Rectangle rectangle) {
		return ((rectangle.getPosture().getX() >= getPosture().getX() + cote)
				|| (rectangle.getPosture().getX() + rectangle.getLargeur() <= getPosture()
						.getX())
				|| (rectangle.getPosture().getY() >= getPosture().getY() + cote) || (rectangle
				.getPosture().getY() + rectangle.getHauteur() <= getPosture()
				.getY())) ? false : true;
	}
	...
	public boolean collisionPoint(double x, double y) {
		return (x > posture.getX() - cote / 2 && x < posture.getX() + cote / 2
				&& y > posture.getY() - cote / 2 && y < posture.getY() + cote
				/ 2) ? true : false;
	}
	...
	public int tangeantPoint(double x, double y) {
		if (Math.abs(x - posture.getX() - cote / 2) <= 0.02)
			if (y >= posture.getY() - cote / 2
					|| y <= posture.getY() + cote / 2)
				return 1;
		if (Math.abs(x - posture.getX() + cote / 2) <= 0.02)
			if (y >= posture.getY() - cote / 2
					|| y <= posture.getY() + cote / 2)
		...
		if (Math.abs(y - posture.getY() - cote / 2) <= 0.02)
			if (x >= posture.getX() - cote / 2
					|| x <= posture.getX() + cote / 2)
				return 2;
		if (Math.abs(y - posture.getY() + cote / 2) <= 0.02)
			if (x >= posture.getX() - cote / 2
					|| x <= posture.getX() + cote / 2)
