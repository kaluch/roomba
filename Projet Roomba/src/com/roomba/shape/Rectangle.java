...
public class Rectangle extends Forme {
	...
	public Rectangle(double x, double y, double theta, double largeur,
			double hauteur) {
		super(x, y, theta);
	...
	void paint(Graphics2D g2, int facteurEchelle, int centrex, int centrey) {
		g2.fillRect(centrex + getxPix(facteurEchelle)
				- (int) (largeur * facteurEchelle / 2), centrey
				+ getyPix(facteurEchelle)
				- (int) (hauteur * facteurEchelle / 2),
				(int) (facteurEchelle * largeur),
				(int) (facteurEchelle * hauteur));
	...
	void draw(Graphics2D g2, int facteurEchelle, int centrex, int centrey) {
		g2.drawRect(centrex + getxPix(facteurEchelle)
				- (int) (largeur * facteurEchelle / 2), centrey
				+ getyPix(facteurEchelle)
				- (int) (hauteur * facteurEchelle / 2),
				(int) (facteurEchelle * largeur),
	...
	public boolean collisionExterne(Carre carre) {
		return ((carre.getPosture().getX() >= getPosture().getX() + largeur)
				|| (carre.getPosture().getX() + carre.getCote() <= getPosture()
						.getX())
				|| (carre.getPosture().getY() >= getPosture().getY() + hauteur) || (carre
				.getPosture().getY() + carre.getCote() <= getPosture().getY())) ? false
				: true;
	}

	public boolean collisionInterne(Cercle cercle) {
		return ((cercle.getPosture().getY() - cercle.getDiametre() / 2 <= getPosture()
				.getY() - getHauteur() / 2)
				|| (cercle.getPosture().getX() - cercle.getDiametre() / 2 <= getPosture()
						.getX() - getLargeur() / 2)
				|| (cercle.getPosture().getY() + cercle.getDiametre() / 2 >= getPosture()
						.getY() + getHauteur() / 2) || (cercle.getPosture()
				.getX() + cercle.getDiametre() / 2 >= getPosture().getX()
				+ getLargeur() / 2)) ? true : false;

	}

	public boolean collisionExterne(Cercle cercle) {
		double xm = (cercle.getPosture().getX() <= getPosture().getX()
				- getLargeur() / 2) ? getPosture().getX() - getLargeur() / 2
				: (cercle.getPosture().getX() <= getPosture().getX()
						+ getLargeur() / 2) ? cercle.getPosture().getX()
						: getPosture().getX() + getLargeur() / 2;
		double ym = (cercle.getPosture().getY() <= getPosture().getY()
				- getHauteur() / 2) ? getPosture().getY() - getHauteur() / 2
				: (cercle.getPosture().getY() <= getPosture().getY()
						+ getHauteur() / 2) ? cercle.getPosture().getY()
						: getPosture().getY() + getHauteur() / 2;
		return (Math.sqrt(Math.pow(cercle.getPosture().getX() - xm, 2)
				+ Math.pow(cercle.getPosture().getY() - ym, 2)) <= cercle
	...
	public boolean collisionExterne(Rectangle rectangle) {
		return ((rectangle.getPosture().getX() >= getPosture().getX() + largeur)
				|| (rectangle.getPosture().getX() + rectangle.getLargeur() <= getPosture()
						.getX())
				|| (rectangle.getPosture().getY() >= getPosture().getY()
						+ hauteur) || (rectangle.getPosture().getY()
				+ rectangle.getHauteur() <= getPosture().getY())) ? false
				: true;
	}
	...
	public boolean collisionPoint(double x, double y) {
		return (x > posture.getX() - largeur / 2
				&& x < posture.getX() + largeur / 2
				&& y > posture.getY() - hauteur / 2 && y < posture.getY()
				+ hauteur / 2) ? true : false;
	}

	@Override
	public int tangeantPoint(double x, double y) {
		if (Math.abs(x - posture.getX() - largeur / 2) <= 0.02)
			if (y >= posture.getY() - hauteur / 2
					|| y <= posture.getY() + hauteur / 2)
				return 1;
		if (Math.abs(x - posture.getX() + largeur / 2) <= 0.02)
			if (y >= posture.getY() - hauteur / 2
					|| y <= posture.getY() + hauteur / 2)
